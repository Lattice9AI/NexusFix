...
public:
    ...
    NFX_FORCE_INLINE ExecutionReportCodec& orderId(std::string_view value) noexcept {
        if (!FixedString20::encode(mutableBody() + Offset::OrderId, value)) { // LCOV_EXCL_BR_LINE: inlined FixedString::encode memcpy/memset dispatch, artifact branch pairs; the logical truncated arm is covered by tests (TICKET_499 WI3)
            truncated_ = true;
        }
        return *this;
    }
    ...
    NFX_FORCE_INLINE ExecutionReportCodec& execId(std::string_view value) noexcept {
        if (!FixedString20::encode(mutableBody() + Offset::ExecId, value)) { // LCOV_EXCL_BR_LINE: inlined FixedString::encode memcpy/memset dispatch, artifact branch pairs; the logical truncated arm is covered by tests (TICKET_499 WI3)
            truncated_ = true;
        }
        return *this;
    }
